...
@click.argument('portal_password')
@click.argument('server_url')
def load_cmd_server_content(portal_url, portal_username, portal_password, server_url):
    _auth = LoginArcgisPortal(portal_url, portal_username, portal_password, server_url)
...
@click.argument('portal_username')
@click.argument('portal_password')
def load_csv_server_content(portal_url, portal_username, portal_password):
    _auth = LoginArcgisPortal(portal_url, portal_username, portal_password)
    _o_content = LoadContentServer(_auth)
    class TThread(threading.Thread):
        def __init__(self, *args, **keywords) -> None:
        ...
        def kill(self):
            raise SystemExit()
    t1 = TThread(target=_o_content.dump_server_to_csv(), daemon=True)
